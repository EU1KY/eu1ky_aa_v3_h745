...
int16_t audioBuf[(NSAMPLES + NDUMMY) * 2] __attribute__((section(".axisram"))) __attribute__((aligned(8)));
...
#pragma GCC diagnostic pop
    ...
    BSP_AUDIO_IN_SelectInterface(AUDIO_IN_INTERFACE_SAI);
    BSP_AUDIO_IN_InitEx(INPUT_DEVICE_INPUT_LINE_1, FSAMPLE, DEFAULT_AUDIO_IN_BIT_RESOLUTION, DEFAULT_AUDIO_IN_CHANNEL_NBR);
    BSP_AUDIO_IN_SetVolume(100 - CFG_GetParam(CFG_PARAM_LIN_ATTENUATION));
...
void DSP_Sample(void)
{
    ...
    HAL_StatusTypeDef res = HAL_SAI_Receive(&haudio_in_sai, (uint8_t*)audioBuf, (NSAMPLES + NDUMMY) * 2, 200);
    if (HAL_OK != res)
    {
