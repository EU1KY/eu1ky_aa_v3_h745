...
#else
#define MEMATTR_OSL __attribute__((section(".axisram")))
#endif
...
float complex OSL_GFromZ(float complex Z, float Rbase)
{
    ...
    float complex G = (Z - Z0) / _cnonz(Z + Z0);
    if (isnan(crealf(G)) || isnan(cimagf(G)) || isinf(crealf(G)) || isinf(cimagf(G)))
    {
