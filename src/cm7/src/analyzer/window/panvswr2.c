...
static void DrawCursorText()
{
    ...
    if (Q > 2000.f)
        Q = 2000.f;
    FONT_ClearLine(FONT_FRAN, LCD_BLACK, Y0 + WHEIGHT + 16);
...
__attribute((unused)) static void DrawSavingText(void)
{
...
__attribute((unused)) static void DrawSavedText(void)
{
...
static void DrawGrid(int drawSwr)  // drawSwr: 0 - R/X, 1 - VSWR, 2 - S11
{
    ...
    for (i = 0; i <= WWIDTH; i++)
    {
        ...
        if (IsFinHamBands(f))
        {
            LCD_VLine(LCD_MakePoint(X0 + i, Y0), WHEIGHT, LCD_RGB(0, 0, 64));
        }
    }
...
#define S11OFFS(s11) ((int)roundf(((s11 - graphmin) * WHEIGHT) / grange) + 1)
    ...
    for (labelValue = graphmin; labelValue < graphmax + (.5 * d); labelValue += d)
    {
        ...
        if (roundf(labelValue) == 0)
            LCD_HLine(LCD_MakePoint(X0, WY(S11OFFS(0.f))), WWIDTH, WGRIDCOLORBR);
        else
            LCD_HLine(LCD_MakePoint(X0, WY(yofs)), WWIDTH, WGRIDCOLOR);
...
static void save_snapshot(void)
{
    ...
    SCREENSHOT_SelectFileName(wbuf);
    SCREENSHOT_DeleteOldest();
    if (CFG_GetParam(CFG_PARAM_SCREENSHOT_FORMAT))
        SCREENSHOT_SavePNG(wbuf);
    else
        SCREENSHOT_Save(wbuf);

    //Now write measured data to S1P file
    sprintf(path, "%s/%s.s1p", sndir, wbuf);
    FIL fo = { 0 };
    UINT bw;
