...
//I2C4 port is wired to Arduino connector
...
extern void     TS_IO_WriteMultiple(uint8_t addr, uint8_t reg, uint8_t* values, uint16_t nvalues);
...
{
    TS_IO_WriteMultiple(CFG_GetParam(CFG_PARAM_SI5351_BUS_BASE_ADDR), addr, data, (uint16_t)bytes);
    return 0;
}
