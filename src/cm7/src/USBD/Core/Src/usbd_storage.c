...
int8_t STORAGE_GetCapacity(uint8_t lun, uint32_t * block_num,
                           uint16_t * block_size)
{
  BSP_MMC_CardInfo info;
  int8_t ret = -1;

  //if (BSP_MMC_IsDetected() != MMC_NOT_PRESENT) // not implemented, it is always present
  {
                    ...
                    uint16_t blk_len)
{
  ...
  //if (BSP_MMC_IsDetected() != MMC_NOT_PRESENT) // not implemented, it is always present
  {
    ...
    SCB_InvalidateDCache_by_Addr((uint32_t*)buf, (int32_t)(512 * blk_len));
    ret = 0;
  }
  return ret;
}
                     ...
                     uint16_t blk_len)
{
  ...
  //if (BSP_MMC_IsDetected() != MMC_NOT_PRESENT) // not implemented, it is always present
  {
    SCB_CleanDCache_by_Addr((uint32_t*)buf, (int32_t)(512 * blk_len));
