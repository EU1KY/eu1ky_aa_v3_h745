...
// Call shell_set_protocol_group(1) to switch to this protocol
...
static bool _nanovna_cmd_exit(uint32_t argc, char* const argv[])
{
    ...
    //shell_set_protocol_group(SHELL_PROT_GROUP_INITIAL);
    return false;
}
SHELL_CMD_PROTO(exit, "exits and then restarts console mode", _nanovna_cmd_exit, SHELL_PROT_GROUP_NANOVNA);
...
SHELL_CMD_PROTO(info, "prints firmware info on console", _nanovna_cmd_info, SHELL_PROT_GROUP_NANOVNA);
...
SHELL_CMD_PROTO(freq, "set the CW frequency", _nanovna_cmd_freq, SHELL_PROT_GROUP_NANOVNA);
...
SHELL_CMD_PROTO(data, "data [array]", _nanovna_cmd_data, SHELL_PROT_GROUP_NANOVNA);
...
SHELL_CMD_PROTO(sweep, "sweep", _nanovna_cmd_sweep, SHELL_PROT_GROUP_NANOVNA);
